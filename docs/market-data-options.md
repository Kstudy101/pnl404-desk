...
조회 기간은 사용자 후속 요청에 따라 일봉 기반7·14·21·28일(기본7일)이다. 주식은 거래일, 암호화폐는 주말을 포함한 달력일로 누적 모멘텀과7일 구간별 변화를 계산한다. 데이터가 바뀌었는지15분마다 확인하고 공급자의 가격 기준 시각과 수집 시각을 구분한다. 장마감 이후 가격이 그대로인 것은 정상이다. 실패 시 마지막 정상 응답을 보존한다.
...
CoinGecko 시총100 목록을 한 요청으로 15분마다 수집하면 30일 기준 `4 × 24 × 30 = 2,880회`다. 현재 Python 모드의 7·14·21·28일 차트는 저장한 일별 이력을 사용한다. 암호화폐 일봉100개를 새 완료일마다 한 번씩 갱신하면 월 약3,000회가 더해져 기본 약5,880회이며, 초기 이력 확보·실패 재시도·구성종목 변경은 별도다. 일반 회차는 최대20개를16초 간격으로 수집하고 미완료분을 다음 회차로 이월한다. 계산 시각을 가격 기준 날짜와 구별한다.

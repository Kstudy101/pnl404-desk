...
| 전광판 | `/api/markets`, `/api/search`, `/api/chart` → `public/modules/board/board.mjs`, `board-model.mjs`, `board.css`, `index.html` | 시장/지수 분류, 자동완성, localStorage 관심종목, 차트, 1시간 갱신 확인 |
...
| 공개 시장 파일 | `public/modules/board/markets.json`·`history/` → Worker·화면 | Python 모드의 주 입력. 3600초 경과 표시를 위해 생성 시각을 임의 갱신하지 않음 |
| 수동 보조 수집 | `scripts/market-refresh.mjs` → `worker/providers.mjs` | JavaScript NAVER/Yahoo/CoinGecko 어댑터. Python 예약 수집과 병행 실행하지 않음 |
| 예약 갱신 | Windows 매시간05분 → `scripts/refresh-all.mjs --deploy` → 원본 생성·Python·검사·게시 | PC 및 사용자 세션 필요. GitHub 시장 전용 수집은 수동 보조만 사용 |
...
| 게시 | `scripts/refresh-all.mjs` → 임시 경로 수집·`publish.mjs` → 검증 → 생성 파일 승격 → 배포 | 실패한 수집·검증은 운영 산출물 미반영. 단독 publish는 준비된 원본 수집용 |
| 배포 | `wrangler.toml` + `.github/workflows/deploy.yml` | Worker 이름 `pnl404-desk`, assets `./public`, main 푸시는 검사만, 운영은 시간별 통합 작업 또는 명시적 수동 배포 |
...
새 시장 항목은 `id/market/symbol/name/groups/currency/price/change_pct/updated_at/source`를 사용하며 결측 가격은 null이다. Python 일봉의 `bar_date`는 거래일 날짜이고 자정 표식을 실제 체결 시각으로 표시하지 않는다. 주식 시가총액은 별도의 출처·시각이다. Python 모드의 Cloudflare Cron은 파일 경과 시간만 확인하며 Python을 실행하지 않는다. 수동 웹 모드 캐시는 24시간 보존과 1시간 신선도를 구분하는 데이터센터별 캐시다. 무료 데이터 수집·종가 중심 요구는 [선택 기록](../../../../docs/market-data-options.md)을 따른다.

암호화폐 일봉 수집은 UTC 완료 경계별로 성공 이력을 재사용한다. 시간별 통합 예약은 최대100종목을16초 간격으로 보충하며 미완료분은 다음 회차로 이월한다. 429 응답은 즉시 중단하고 Retry-After 및 지수 대기 시간을 산출물에 기록한다. 가격·모멘텀 부분 성공은 종료 코드2이며 통합 실행기는 이를 명시적으로 허용한 뒤 산출물 검사로 값·이력·계산 계약을 확인한다. 전체 실패는 게시하지 않는다.
